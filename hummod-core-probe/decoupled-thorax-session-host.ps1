...
$allowedSetSymbols=@{
  ...
  'Thorax.CoupledPressureSwitch'=@{min=0.0;max=1.0;integer=$true}
  'Thorax.CoupledPressure'=@{min=-10.0;max=19.0;integer=$true}
}
...
function Apply-LiveControls($assignments) {
  ...
  if(@($assignments.PSObject.Properties |
      Where-Object { $coupledThoraxNames -contains $_.Name }).Count){
    Write-LiveControlStage 'coupled-thorax-panel-opening'
    Open-Panel '/Physiology/Lungs/Thorax'
    Write-LiveControlStage 'coupled-thorax-panel-open'

    $children=@([HumModHostNative]::Children($main.Handle))
    $horizontal=@($children | Where-Object {
      $_.Class -eq 'ScrollBar' -and $_.Width -gt $_.Height
    } | Sort-Object Top)

    # Pinned stock panel has right/left NormalPressure controls high on the
    # panel. The bridge patch adds one additional lower horizontal scrollbar.
    if($horizontal.Count -lt 3){
      throw "Expected patched Thorax panel to expose at least 3 horizontal scrollbars; found $($horizontal.Count)"
    }
    ...
    if($buttons.Count -lt 2){
      throw "Cannot identify patched Thorax bridge switch buttons; found $($buttons.Count)"
    }

    if($null -ne $assignments.'Thorax.CoupledPressureSwitch'){
      $v=[int]$assignments.'Thorax.CoupledPressureSwitch'
      if($v -ne 0 -and $v -ne 1){
        throw 'Thorax.CoupledPressureSwitch must be 0 or 1'
      }
      $off=$buttons[$buttons.Count-2]
      $on=$buttons[$buttons.Count-1]
      Click-LiveButton ($(if($v -eq 1){$on.Id}else{$off.Id})) 'Thorax.CoupledPressureSwitch'
      $diag['Thorax.CoupledPressureSwitch']=[ordered]@{
        offId=$off.Id
        onId=$on.Id
        requested=$v
      }
    ...
    if($null -ne $assignments.'Thorax.CoupledPressure'){
      $v=[double]$assignments.'Thorax.CoupledPressure'
      if($v -lt -10 -or $v -gt 19 -or $v -ne [math]::Round($v)){
        throw 'Thorax.CoupledPressure live control supports integer -10..19 mmHg'
      }
      $range=[HumModHostNative]::ScrollRange($bridgeScroll.Handle)
      $position=[int]($v+10)
      if($position -lt $range[0] -or $position -gt $range[1]){
        throw "Thorax.CoupledPressure position outside native scrollbar range"
      }
      [HumModHostNative]::SetScroll($bridgeScroll.Handle,$position)
      Start-Sleep -Milliseconds 150
      $diag['Thorax.CoupledPressure']=[ordered]@{
        id=$bridgeScroll.Id
        requestedValue=$v
        requestedPosition=$position
        actualPosition=[HumModHostNative]::ScrollPos($bridgeScroll.Handle)
        minimum=$range[0]
        maximum=$range[1]
      }
    }
