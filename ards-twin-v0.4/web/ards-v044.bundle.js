var VENT = (() => {
  ...
  var require_recruitment = __commonJS({
    "src/recruitment.js"(exports, module) {
      ...
      function recruitmentRate(recruitment, pDist, params = {}) {
        var _a, _b, _c, _d;
        const Popen = (_a = params.P_open) != null ? _a : OPEN_DEFAULT;
        const Pclose = (_b = params.P_close) != null ? _b : CLOSE_DEFAULT;
        const kopen = (_c = params.k_open) != null ? _c : K_OPEN_DEFAULT;
        const kclose = (_d = params.k_close) != null ? _d : K_CLOSE_DEFAULT;
        if (!(Pclose < Popen)) {
    ...
    "src/mechanics.js"(exports, module) {
      ...
      function solveImplicitStep(params, state, boundary, dt, recruitmentSnapshot) {
        ...
        for (let i = 0; i < params.compartments.length; i++) {
          ...
          const csWithR = __spreadProps(__spreadValues({}, cs), { recruitment: recruitmentSnapshot[i] });
        ...
        if (activeComps.length === 0) {
          const zero = state.compartments.map((cs) => __spreadProps(__spreadValues({}, cs), {
            volume: 0,
            flow: 0,
          ...
          return {
            state: __spreadProps(__spreadValues({}, state), { compartments: zero, totalVolume, totalFlow: 0 }),
            output: {
        ...
        if (dt / 2 < 1e-6) {
          ...
          return {
            state: {
              t: state.t,
              compartments: state.compartments.map((cs, i) => __spreadProps(__spreadValues({}, cs), {
      ...
      function finalize(activeComps, vTrial, pBranchSolved, state, boundary, params, dt, iterations, residualNorm, scaledResidual, substeps, lineSearchHalvings, activeSetTransitions) {
        ...
        const nextComps = state.compartments.map((cs, i) => {
          ...
          if (!ac) {
            return __spreadProps(__spreadValues({}, cs), { volume: 0, flow: 0, alveolarPressure: aop });
          }
          ...
          return __spreadProps(__spreadValues({}, cs), { volume: vNew, flow: q, alveolarPressure: pAlv });
        });
      ...
      var ThreeCompartmentMechanics = class {
        step(params, state, boundary, dt) {
          ...
          for (let i = 0; i < compartments.length; i++) {
            if (Math.abs(rCorrect[i] - rPredict[i]) > RECRUITMENT_TOL) {
              const newState = __spreadProps(__spreadValues({}, state), {
                compartments: state.compartments.map((cs, idx) => __spreadProps(__spreadValues({}, cs), {
          ...
          final.state.compartments = final.state.compartments.map((cs, i) => __spreadProps(__spreadValues({}, cs), {
          ...
          const state = {
            ...
            compartments: compartments.map((cs, i) => {
              ...
              return {
                ...
                flow: 0,
                alveolarPressure: 0,
                recruitment: (_a = cs.recruitment) != null ? _a : 1
              };
            }),
            airwayPressure: 0,
            totalFlow: 0,
  ...
  var require_contracts = __commonJS({
    "src/contracts.js"(exports, module) {
      ...
      function cloneState(state) {
        return {
          t: state.t,
          compartments: state.compartments.map((s) => __spreadValues({}, s)),
      ...
      function makeInitialState2(params, options = {}) {
        function pickRecState() {
          ...
          throw new Error(
            "makeInitialState: initialRecruitmentState is required (caller must pass it via options or as part of the phenotype contract)."
          );
        }
        function pickPEEP() {
        ...
        if (peep === null) {
          throw new Error(
            "makeInitialState: initialPEEP is required (caller or controller must provide it). The initializer does not guess initial PEEP."
          );
        }
        const aop = params.airwayOpeningPressure;
        ...
        function availabilityFor(cp) {
          ...
          if (cp.id === "recruitable") {
            ...
            return Math.max(0, Math.min(1, (_a = recState.recruitable) != null ? _a : 0));
          }
          throw new Error(`unknown compartment id: ${cp.id}`);
        }
        const compartments = params.compartments.map((cp) => {
          ...
          if (vmax <= 0) {
            ...
            volume = forwardElasticVolume2(peep, __spreadProps(__spreadValues({}, cp), { capacity: vmax }), a, aop);
          } else {
            volume = 0;
          }
      ...
      var Simulation2 = class {
        constructor({
          ...
          if (initialRecruitmentState) {
            ...
            throw new Error(
              "Simulation: initializationHistory recovery is reserved for a future release; pass initialRecruitmentState explicitly."
            );
          } else {
            throw new Error(
              "Simulation: scenario must provide initialRecruitmentState (or initializationHistory). The phenotype does not own recruitment."
            );
          }
  ...
  __export(bundle_entry_exports, {
    PRESETS: () => import_presets.PRESETS,
    PcAcController: () => import_simulation.PcAcController,
    Simulation: () => import_simulation.Simulation,
    VcAcController: () => import_simulation.VcAcController,
    dPressureDVolume: () => import_compartments.dPressureDVolume,
    effectiveVolumeCapacity: () => import_compartments.effectiveVolumeCapacity,
    forwardElasticVolume: () => import_compartments.forwardElasticVolume,
    makeBoundaryFlow: () => import_contracts.makeBoundaryFlow,
    makeBoundaryPressure: () => import_contracts.makeBoundaryPressure,
    makeInitialState: () => import_contracts.makeInitialState,
    makePatientParams: () => import_contracts.makePatientParams
  });
