...
function main(){
  ...
  const report={
    ...
    calibration:{
      ...
      nativeCirculationStateAvailable:Boolean(nativeCalibrationTarget && nativeCalibrationTarget.nativeCirculationState && nativeCalibrationTarget.nativeCirculationState.available),
      nativeCirculationStateApplied:Boolean(snapshot.coupling.nativeCirculationStateApplied),
    ...
    applicability:{
      ...
      matchedNativeControls:['FiO2','respiratory-rate','tidal-volume','heart-rate-when-native-target-provided','gas-state-initialization-when-native-state-symbols-available','circulation-compartment-volumes-when-native-state-symbols-available'],
