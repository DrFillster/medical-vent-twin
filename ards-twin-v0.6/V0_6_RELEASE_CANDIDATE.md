...
Branch: `product/v0.6-deployable-clinical-twin`
...
The browser UI previously sent the systemic provider as `data.provider`, while the clinical worker inspected `payload.systemicMode`. That could route a requested live session into the replay constructor. v0.6 fixes the worker to route `live-reduced-hummod` explicitly to `createBerlinLiveHumModSession`.

A dedicated `v0_6_product_smoke.test.js` now exercises live initialization, time advance, persistent PEEP change, and finite cardiopulmonary outputs.
